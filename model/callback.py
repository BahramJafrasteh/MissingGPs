...
class logger():
    def __init__(self, model, train_generator , test_generator, on_epoch_end_file,
                 on_train_end_file, device = None, lastlayer=False, reset_logger=True):
        ...
        if reset_logger:
            with open(on_epoch_end_file, "w") as myfile:
                myfile.write('epoch, nelbo_train, acc_train, nll_train, acc_train_c, nll_train_c, nelbo_test, acc_test, nll_test, acc_test_c, nll_test_c,total_time_train, prediction_time' '\n')
            with open(on_train_end_file, "w") as myfile:
                myfile.write('RMSE_train, NLL_train, RMSE_train_c, NLL_train_c, RMSE_test, NLL_test, RMSE_test_c, NLL_test_c,total_training_time, prediction_time' '\n')
    ...
    def evaluate(self, generator, normalize_y=True):
        ...
        with torch.no_grad():
            ...
            kbar.update(0, values=[("nelbo", 0), ("rmse", 0), ("nll", 0), ("rmse_c", 0), ("nll_c", 0)])
            # idx += 1
            avg_nelbo = 0
            avg_acc = 0
            avg_nll = 0
            ...
            for idx, data in enumerate(generator):
                ...
                loss = self.model.test_step(batch_x.to(self.device), batch_y.to(self.device), batch_ind_nan,
                                            batch_ind_nan_target, normalize_y=True)
                ...
                if idx == 0:
                    ...
                    if self.model.opt.missing:
                        ...
                        mean_pred_aggr = torch.cat((mean_pred_aggr, self.model.mean_pred_aggr), 1)
                        std_pred_aggr = torch.cat((std_pred_aggr, self.model.std_pred_aggr), 1)
                ...
                kbar.update(idx + 1, values=[("nelbo", loss), ("rmse", acc), ("nll", nll),("rmse_c", acc_c), ("nll_c", nll_c)])
                # idx += 1
        NLL, ACC = compute_nn_mse(y_aggr, mean_pred_aggr, std_pred_aggr, ind_nan_aggr)
        NLL = NLL.detach().cpu().numpy()
        ACC = ACC.detach().cpu().numpy()
        NLL_c, ACC_c = compute_nn_mse(y_aggr*self.model.y_std+self.model.y_mean, mean_pred_aggr*self.model.y_std+self.model.y_mean, std_pred_aggr*self.model.y_std, ind_nan_aggr)
        ...
        return NELBO, NLL, ACC, NLL_c, ACC_c, [x_aggr, y_aggr, mean_pred_aggr, std_pred_aggr, ind_nan_aggr]
    ...
    def on_epoch_end(self, ini_time, message_write ={}, predict_test = True):
        ...
        if predict_test:
            NELBO, NLL, ACC, NLL_c, ACC_c, _ = self.evaluate(self.test_generator, normalize_y=False)
        ...
        with open(self.on_epoch_end_file, "a") as myfile:
            myfile.write(str(message_write['epoch']) + " " +
    ...
    def on_eval(self, training_gen = None, test_gen = None):
        ...
        if not self.model.opt.missing:
            ...
            end_predict = time.time()
            NELBO_test, NLL_test, ACC_test, NLL_test_c, ACC_test_c, [x_aggr_test, y_aggr_test, mean_pred_aggr_test, std_pred_aggr_test, ind_nan_aggr_test] = self.evaluate(self.test_generator, normalize_y=False)

            print('\n')


            NELBO_train, NLL_train, ACC_train, NLL_train_c, ACC_train_c, [x_aggr_train, y_aggr_train, mean_pred_aggr_train, std_pred_aggr_train, ind_nan_aggr_train] = self.evaluate(self.train_generator, normalize_y=False)
        ...
        print('Training time {}, prediction time {}'.format(self.total_train_time, prediction_time))
        ...
        with open(self.on_train_end_file, "a") as myfile:
            myfile.write(str(ACC_train) + " " +
                         ...
                         str(self.total_train_time) + " "+
                         str(prediction_time)  + '\n')
        return NELBO_train, NLL_train, ACC_train, NELBO_test, NLL_test, ACC_test, [x_aggr_train, y_aggr_train, mean_pred_aggr_train, std_pred_aggr_train, ind_nan_aggr_train], \
               [x_aggr_test, y_aggr_test, mean_pred_aggr_test, std_pred_aggr_test, ind_nan_aggr_test]
